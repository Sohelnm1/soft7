...
export async function processWebhookQueue(batchSize: number = 500) {
    ...
    const campaignStats: Record<string, { delivered: number; read: number; failed: number }> = {};
    ...
    for (const hook of webhooks) {
        ...
        if (value.statuses?.[0]) {
            ...
            statusUpdates.push({ wamid, status, timestamp, errorCode: statusObj.errors?.[0]?.code, errorMessage: statusObj.errors?.[0]?.message });
    ...
    for (const update of statusUpdates) {
        ...
        const io = getIO();
        if (io) {
            io.emit("message_status_update", { wamid: update.wamid, status: update.status });
        }
    }
